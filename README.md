...
`prepare-research-results-template` turns a private research plan into a fillable JSON file under `data/research-results-templates/`. It copies deal IDs, company names, provider IDs, source kinds, and licensing notes from the plan, but leaves fact fields and citation URLs blank so the file cannot be mistaken for validated evidence.
