...
- PR #33: OCR-needed image files should be ingested as explicit low-quality OCR and vision-needed documents rather than skipped, image-only deals should remain evidence-less with operator warnings, and PDF OCR warnings should distinguish image-backed or empty pages from one-off divider pages.
